...
class AldiDEScraper extends ScraperBase
{
    public function scrape(): array
    {
        $products = [];
        echo "  Scrapen van folders van ALDI (DE)...\n";

        $html = $this->fetchUrl('https://www.aldi.de/angebote.html');
    ...
    private function processItems(array $items, array &$products): array
    {
        ...
        foreach ($items as $item) {
            ...
            $products[] = ['name' => $name, 'price' => $price, 'store' => 'Aldi (DE)', 'category' => $category];
        }
        return $products;
    }
    ...
    private function scrapeFallback(string $html, array &$products): array
    {
        ...
        preg_match_all('/<div[^>]*class="[^"]*article-tile[^"]*"[^>]*>(.*?)<\/div>/s', $html, $tiles);
        foreach ($tiles[1] as $tile) {
            $name = '';
            if (preg_match('/<h[23][^>]*>(.*?)<\/h[23]>/s', $tile, $m)) {
    ...
    private function categorizeProduct(string $name): ?string
    {
        ...
        $map = [
            ...
            'getränk' => 'dranken', 'wasser' => 'dranken', 'bier' => 'dranken',
