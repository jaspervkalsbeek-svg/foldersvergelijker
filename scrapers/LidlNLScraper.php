...
class LidlNLScraper extends ScraperBase
{
    public function scrape(): array
    {
        $products = [];
        echo "  Scrapen van folders van Lidl (NL)...\n";

        $html = $this->fetchUrl('https://www.lidl.nl/aanbiedingen');
        ...
        $walk = function ($node) use (&$walk, &$items) {
            ...
            if (isset($node['name']) && isset($node['price'])) {
                $items[] = [
                    ...
                    'image'    => $node['image'] ?? $node['imageUrl'] ?? null,
    ...
    private function processItems(array $items, array &$products): array
    {
        ...
        foreach ($items as $item) {
            ...
            if ($price > 0) $this->savePrice($pid, $price, $unitSize, null);

            $products[] = ['name' => $name, 'price' => $price, 'store' => 'Lidl', 'category' => $category];
        }
        return $products;
    }
    ...
    private function scrapeFallback(string $html, array &$products): array
    {
        ...
        preg_match_all('/<div[^>]*class="[^"]*product[^"]*"[^>]*>(.*?)<\/div>/s', $html, $divs);
    ...
    private function categorizeProduct(string $name): ?string
    {
        ...
        $map = [
            'milch' => 'zuivel-eieren', 'kaas' => 'zuivel-eieren', 'joghurt' => 'zuivel-eieren',
            'brot' => 'brood-ontbijtgranen', 'brötchen' => 'brood-ontbijtgranen',
            'obst' => 'fruit-groente', 'gemüse' => 'fruit-groente', 'apfel' => 'fruit-groente',
            'fleisch' => 'vlees-vis', 'hähnchen' => 'vlees-vis', 'wurst' => 'vlees-vis',
